...
const Page = () => {
  ...
  useEffect(() => {
    async function getTaskFromCategory() {
      await getCategoryAndTasks(categoryId).then((response) => {
    ...
    <>
      <div className="flex items-center flex-col">
        ...
        <p className="font-roboto font-medium text-lg my-4 mx-4 px-2 text-center">
          ...
          <span className="text-teal-600">tareas</span> de la categoria{" "}
          <span className={`text-${colorCategory}`}>{category.name}</span>
        </p>
        {tasks.length > 0 ? (
          <>
